 ...
 */
function bronze_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'bronze_add_menu_parent_class' );
 ...
 */
function bronze_nav_menu_social_icons( $item_output, $item, $depth, $args ) {

	$social_icons = bronze_social_links_icons();
 ...
 */
function bronze_menu_item_markup( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'bronze_menu_item_markup', 10, 4 );
 ...
 */
function bronze_add_menu_item_link_attributes( $atts, $item, $args ) {
...
add_filter( 'nav_menu_link_attributes', 'bronze_add_menu_item_link_attributes', 10, 3 );
 ...
 * @return array $classes
 */
function bronze_add_menu_item_css_classes( $classes, $item, $args ) {
...
add_filter( 'nav_menu_css_class', 'bronze_add_menu_item_css_classes', 10, 3 );

/**
 * Add a cart menu item in mobile menu
 */
function bronze_add_cart_menu_item_mobile( $items, $args ) {
	...
	if ( bronze_display_cart_menu_item() ) {
		if ( $args->theme_location === 'primary' && preg_match( '/mobile/', $args->menu_class ) ) {
			...
			$items .= bronze_cart_menu_item_mobile( false );
			$items .= '</li>';
		}
	}
...
add_filter( 'wp_nav_menu_items', 'bronze_add_cart_menu_item_mobile', 10, 2 );
 ...
 * @param string $location
 */
function bronze_get_menu_args( $location = 'primary', $type = 'desktop', $depth = 3 ) {
	...
	$args = array(
		...
		'depth'          => $depth,
		'fallback_cb'    => 'bronze_menu_fallback',
 ...
 */
function bronze_menu_fallback( $args ) {
	...
	$link = $link_before
	. '<a class="no-link-style" href="' . admin_url( 'nav-menus.php' ) . '">' . $before . esc_html__( 'Add Menu', 'bronze' ) . $after . '</a>'
	. $link_after;
 ...
 */
function bronze_social_links_icons() {
	...
	return apply_filters( 'bronze_social_links_icons', $social_links_icons );
}
