<?php
/**
 * Bronze background functions
 *
 * @author WolfThemes
 * @category Core
 * @package Bronze/FRontend
 * @version 1.1.0
 */
 ...
 */
function bronze_background_overlay( $args ) {
	...
	if ( ( 'custom' === $overlay_color || 'auto' === $overlay_color ) && $overlay_custom_color ) {
		...
		$overlay_style .= 'background-color:' . bronze_sanitize_color( $overlay_custom_color ) . ';';
	...
	return '<div style="' . bronze_esc_style_attr( $overlay_style ) . '" class="' . bronze_sanitize_html_classes( $class ) . '"></div><!--.wvc-bg-overlay-->';
}
 ...
 * @param array $args
 */
function bronze_background_img( $args = array() ) {
	...
	$do_object_fit = ( wp_attachment_is_image( $background_img ) && 'no-repeat' === $background_repeat && 'default' !== $background_size && 'parallax' !== $background_effect && ! bronze_is_edge() && ! wp_is_mobile() );
	...
	if ( $do_object_fit ) {
		...
		$src                = bronze_get_url_from_attachment_id( $background_img, $background_img_size );
		...
		$img_dominant_color = bronze_get_image_dominant_color( $background_img );
		...
		$output .= '<div class="' . bronze_sanitize_html_classes( $container_class ) . '" style="' . bronze_esc_style_attr( $container_style ) . '">';
		...
		$output .= '<img
			...
			style="' . bronze_esc_style_attr( $cover_style ) . '"
		...
		$output .= ' class="' . bronze_sanitize_html_classes( $cover_class ) . '"
		...
		if ( 'parallax' === $background_effect && wp_attachment_is_image( $background_img ) ) {
			...
			$background_color = bronze_get_image_dominant_color( $background_img );
		...
		if ( 'parallax' !== $background_effect ) {
			...
			if ( wp_attachment_is_image( $background_img ) ) {
				$background_img_url = bronze_get_url_from_attachment_id( $background_img, $background_img_size );
				$style             .= 'background-image:url(' . esc_url( $background_img_url ) . ');';
			}
		}

		$output .= '<div ' . $attrs . ' class="' . bronze_sanitize_html_classes( $container_class ) . '" style="' . esc_attr( $style ) . '"></div>';

	} elseif ( bronze_is_instagram_post() ) {

		$container_class = 'img-bg';

		$instagram_url = bronze_get_instagram_image_url();
		$output       .= '<div class="' . bronze_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $instagram_url ) . ');"></div>';
		...
		$bg_fallback_url = bronze_get_placeholder_url();

		$output .= '<div class="' . bronze_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $bg_fallback_url ) . ');"></div>';
	}
 ...
 * @return string $output
 */
function bronze_background_slideshow( $args = array() ) {
	...
	$do_object_fit = ( ! bronze_is_edge() && ! wp_is_mobile() );
	...
	if ( array() !== $slideshow_img_ids && is_array( $slideshow_img_ids ) ) {
		...
		foreach ( $slideshow_img_ids as $image_id ) {
			...
			$src = esc_url( bronze_get_url_from_attachment_id( $image_id, $slideshow_image_size ) );

			$output .= '<li class="slide">';

			if ( $do_object_fit ) {
				$output .= bronze_resized_thumbnail( $slideshow_image_size, 'cover', $image_id, false );
 ...
 */
function bronze_background_video( $args = array(), $vimeo = true ) {

	$args = wp_parse_args(
		$args,
		array(
			'video_bg_url'        => bronze_get_first_video_url(),
	...
	if ( 'selfhosted' === bronze_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= bronze_video_bg( $args );

	} elseif ( 'youtube' === bronze_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= bronze_youtube_video_bg( $args );

	} elseif ( 'vimeo' === bronze_get_video_url_type( $args['video_bg_url'] ) && $vimeo ) {

		$output .= bronze_vimeo_video_bg( $args );
	}
...
function bronze_video_bg( $args ) {
	...
	if ( $video_bg_img ) {
		$output .= bronze_resized_thumbnail( $video_bg_img_size, 'cover video-bg-fallback', $video_bg_img, false );
	}
 ...
 * @return string $output
 */
function bronze_youtube_video_bg( $args ) {
	...
	wp_enqueue_script( 'bronze-youtube-video-background' );
 ...
 * @return string $output
 */
function bronze_vimeo_video_bg( $args ) {
	...
	wp_enqueue_script( 'bronze-vimeo' );
