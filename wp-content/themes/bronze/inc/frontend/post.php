 ...
 *
 * A function that returns any loop of any post type and push it to the bronze_posts hook
 * Cool right?
 *
 * @package WordPress
 * @subpackage Bronze
 ...
 * @return void
 */
function bronze_output_posts( $atts ) {
	...
	$atts = wp_parse_args(
		...
		array(
			...
			/* Common attributes */
			'grid_padding'                           => bronze_get_theme_mod( 'post_grid_padding', 'yes' ),
			'item_animation'                         => bronze_get_theme_mod( 'post_item_animation' ),
			'columns'                                => 'default',
			'include_ids'                            => '',
			'exclude_ids'                            => '',
			...
			/* Post */
			'post_index'                             => false,
			'post_display'                           => bronze_get_theme_mod( 'post_display', 'standard' ),
			'post_metro_pattern'                     => 'auto',
			'post_module'                            => 'grid',
			'post_thumbnail_size'                    => 'standard',
			'post_layout'                            => 'standard',
			'post_category_filter'                   => bronze_get_theme_mod( 'post_category_filter', false ),
			'post_excerpt_type'                      => 'auto', // for standard post type
			'post_excerpt_length'                    => 'shorten',
			'post_display_elements'                  => bronze_get_theme_mod( 'post_display_elements', 'show_thumbnail,show_date,show_text,show_author,show_category' ),
			...
			/* Works */
			'work_index'                             => false,
			'work_display'                           => bronze_get_theme_mod( 'work_display', 'grid' ),
			'work_module'                            => 'grid',
			'work_thumbnail_size'                    => 'standard',
			'work_custom_thumbnail_size'             => '',
			'work_layout'                            => 'overlay',
			'work_is_gallery'                        => '',
			'work_category_filter'                   => bronze_get_theme_mod( 'work_category_filter', true ),
			...
			/* Products */
			'product_index'                          => false,
			'product_display'                        => bronze_get_theme_mod( 'product_display', 'grid_classic' ),
			...
			/* Releases */
			'release_index'                          => false,
			'release_display'                        => bronze_get_theme_mod( 'release_display', 'grid' ),
			'release_metro_pattern'                  => 'pattern-1',
			'release_hover_effect'                   => apply_filters( 'release_default_hover_effect', 'default' ),
			'release_category_filter'                => bronze_get_theme_mod( 'release_category_filter', false ),
			...
			/* Events */
			'event_index'                            => false,
			'event_display'                          => bronze_get_theme_mod( 'event_display', 'list' ),
			'event_module'                           => 'grid',
			...
			'mp_event_display'                       => bronze_get_theme_mod( 'mp_event_display', 'grid' ),
			...
			/* Videos */
			'video_index'                            => false,
			'video_display'                          => bronze_get_theme_mod( 'video_display', 'grid' ),
			'video_layout'                           => 'overlay',
			'video_module'                           => 'grid',
			'video_preview'                          => false,
			'video_onclick'                          => bronze_get_theme_mod( 'video_onclick', 'lightbox' ),
			'video_category_filter'                  => bronze_get_theme_mod( 'video_category_filter', false ),
			...
			/* Artists */
			'artist_index'                           => false,
			'artist_display'                         => bronze_get_theme_mod( 'artist_display', 'list' ),
			'artist_metro_pattern'                   => 'auto',
			'artist_hover_effect'                    => apply_filters( 'artist_default_hover_effect', 'default' ),
			'artist_module'                          => 'grid',
			'artist_category_filter'                 => bronze_get_theme_mod( 'artist_category_filter', false ),
			...
			/* Albums */
			'gallery_index'                          => false,
			'gallery_display'                        => bronze_get_theme_mod( 'gallery_display', 'grid' ),
			'gallery_module'                         => 'grid',
			'gallery_thumbnail_size'                 => 'standard',
			'gallery_custom_thumbnail_size'          => 'standard',
			'gallery_layout'                         => 'standard',
			'gallery_category_filter'                => bronze_get_theme_mod( 'gallery_category_filter', true ),
			...
			'attachment_display'                     => bronze_get_theme_mod( 'attachment_display', 'masonry_horizontal' ),
	...
	$atts = apply_filters( 'bronze_post_module_atts', $atts );
	...
	$layout = apply_filters( 'bronze_post_module_layout', $layout, $atts );
	...
	$display = apply_filters( 'bronze_post_module_display', $display, $atts );
	$metro_pattern = ( isset( ${$post_type . '_metro_pattern'} ) ) ? ${$post_type . '_metro_pattern'} : 'auto';
	$metro_pattern = apply_filters( 'bronze_post_module_metro_pattern', $metro_pattern, $atts );
	$module = ( isset( ${$post_type . '_module'} ) ) ? ${$post_type . '_module'} : 'grid';
	$module = apply_filters( 'bronze_post_module_module', $module, $atts );
	$category_filter = ( isset( ${$post_type . '_category_filter'} ) ) ? bronze_attr_bool( ${$post_type . '_category_filter'} ) : false;
	...
	$custom_thumbnail_size = apply_filters( 'bronze_post_module_custom_thumbnail_size', $custom_thumbnail_size, $display, $atts );

	$is_index = ( isset( ${$post_type . '_index'} ) ) ? bronze_attr_bool( ${$post_type . '_index'} ) : false;
	...
	$args['post__in'] = array();
	if ( $include_ids ) {
		$args['post__in'] = bronze_list_to_array( $include_ids );
	}
	$exclude_ids_array   = array();
	$exclude_ids_array[] = bronze_get_the_id(); // exclude current post, obviously or the internet will explode.
	...
	if ( $exclude_ids ) {
		$exclude_ids_array = array_merge( $exclude_ids_array, bronze_list_to_array( $exclude_ids ) );
	}
	...
	*/
	...
	if ( 'post' === $post_type ) {
		...
		if ( $category ) {
			$args['category_name'] = bronze_clean_list( $category );
		}

		if ( $category_exclude ) {
			$args['category__not_in'] = array( bronze_clean_list( $category_exclude ) );
		}
		if ( $tag ) {
			$args['tag'] = bronze_clean_list( $tag );
		}

		if ( $tag_exclude ) {
			$args['tag__not_in'] = bronze_clean_list( $tag_exclude );
		}
	...
	*/
	...
	if ( 'work' === $post_type ) {
		if ( $work_type_include ) {
			$args['work_type'] = bronze_clean_list( $work_type_include );
		}
		if ( $work_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'work_type',
					'terms'    => array( bronze_clean_list( $work_type_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'video' === $post_type ) {
		...
		if ( $video_type_include ) {
			$args['video_type'] = bronze_clean_list( $video_type_include );
		}
		if ( $video_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'video_type',
					'terms'    => array( bronze_clean_list( $video_type_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $video_tag_include ) {
			$args['video_tag'] = bronze_clean_list( $video_tag_include );
		}
		if ( $video_tag_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'video_tag',
					'terms'    => array( bronze_clean_list( $video_tag_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'mp-event' === $post_type ) {
		if ( $mp_event_category_include ) {
			$args['mp-event_category'] = bronze_clean_list( $mp_event_category_include );
		}
		if ( $mp_event_category_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'mp-event_category',
					'terms'    => array( bronze_clean_list( $mp_event_category_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $mp_event_tag_include ) {
			$args['mp-event_tag'] = bronze_clean_list( $mp_event_tag_include );
		}
		if ( $mp_event_tag_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'mp-event_tag',
					'terms'    => array( bronze_clean_list( $mp_event_tag_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'gallery' === $post_type ) {
		if ( $gallery_type_include ) {
			$args['gallery_type'] = bronze_clean_list( $gallery_type_include );
		}
		if ( $gallery_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'gallery_type',
					'terms'    => array( bronze_clean_list( $gallery_type_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'release' === $post_type ) {
		if ( $band_include ) {
			$args['band'] = bronze_clean_list( $band_include );
		}
		if ( $band_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'band',
					'terms'    => array( bronze_clean_list( $band_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $label_include ) {
			$args['label'] = bronze_clean_list( $label_include );
		}
		if ( $label_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'label',
					'terms'    => array( bronze_clean_list( $label_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $genre_include ) {
			$args['genre'] = bronze_clean_list( $genre_include );
		}
		if ( $genre_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'release_genre',
					'terms'    => array( bronze_clean_list( $genre_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'artist' === $post_type ) {
		if ( $artist_genre_include ) {
			$args['artist_genre'] = bronze_clean_list( $artist_genre_include );
		}
		if ( $artist_genre_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'artist_genre',
					'terms'    => array( bronze_clean_list( $artist_genre_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'event' === $post_type ) {
		...
		if ( $artist_include ) {
			$args['we_artist'] = bronze_clean_list( $artist_include );
		}
		if ( $artist_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'we_artist',
					'terms'    => array( bronze_clean_list( $artist_exclude ) ),
					'field'    => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	*/
	...
	if ( 'product' === $post_type ) {
		...
		if ( $product_cat ) {
			$args['product_cat'] = bronze_clean_list( $product_cat );
		}
	...
	*/
	...
	if ( 'attachment' === $post_type ) {
		...
		if ( isset( $_GET['s'] ) ) {
			...
			$tax_query = new WP_Query( apply_filters( 'bronze_post_module_query_args', $args ) );
	...
	if ( $is_index ) { // is index page.
		...
		add_filter( 'found_posts', 'bronze_offset_pagination_fix', 10, 2 );
		...
		remove_filter( 'found_posts', 'bronze_offset_pagination_fix', 10, 2 );
	}

	/**
	 * Add action before the output
	 */
	do_action( 'bronze_before_post_module', $atts );
	if ( $query->have_posts() ) {
		...
		if ( $category_filter ) {
			...
			set_query_var(
				...
				array(
					...
					/* Post */
					'category'                             => bronze_clean_list( $category ),

					/* Work */
					'work_type_include'                    => bronze_clean_list( $work_type_include ),
					'work_type_exclude'                    => bronze_clean_list( $work_type_exclude ),
					'work_category_filter_text_alignment'  => $work_category_filter_text_alignment,

					/* Albums */
					'gallery_type_include'                 => bronze_clean_list( $gallery_type_include ),
					'gallery_type_exclude'                 => bronze_clean_list( $gallery_type_exclude ),
					'gallery_category_filter_text_alignment' => $gallery_category_filter_text_alignment,

					/* Video */
					'video_type_include'                   => bronze_clean_list( $video_type_include ),
					'video_type_exclude'                   => bronze_clean_list( $video_type_exclude ),
					'video_category_filter_text_alignment' => $video_category_filter_text_alignment,

					/* Artist */
					'artist_genre_include'                 => bronze_clean_list( $artist_genre_include ),
					'artist_genre_exclude'                 => bronze_clean_list( $artist_genre_exclude ),
					'artist_category_filter_text_alignment' => $artist_category_filter_text_alignment,

					/* Release */
					'band_include'                         => bronze_clean_list( $band_include ),
					'band_exclude'                         => bronze_clean_list( $band_exclude ),
			 ...
			 * Category filter
			 */
			get_template_part( bronze_get_template_dirname() . '/components/filter/filter', $post_type );
		}

		$tag = ( $is_list ) ? 'ul' : 'div';
		echo '<' . esc_attr( $tag ) . ' id="' . esc_attr( $id ) . '" data-post-type="' . esc_attr( $post_type ) . '" data-params="' . esc_js( $json_params ) . '" class="' . bronze_sanitize_html_classes( $class ) . '"';

		if ( bronze_esc_style_attr( $inline_style ) ) {
			echo ' style="' . bronze_esc_style_attr( $inline_style ) . '"';
		}
		...
		add_filter(
			'bronze_post_module_layout',
			function( $default ) use ( $layout ) {
				return $layout;
			}
		);
		add_filter(
			'bronze_post_item_animation',
			function( $default ) use ( $item_animation ) {
				return bronze_animation_fallback( $item_animation );
			}
		);
		add_filter(
			'bronze_post_columns',
		...
		while ( $query->have_posts() ) {
			 ...
			 */
			get_template_part( bronze_get_template_dirname() . '/components/' . $post_type . '/display/content', $display );

		}

		echo '</' . esc_attr( $tag ) . '><!--.items-->';
		remove_all_filters( 'bronze_post_display' );
		remove_all_filters( 'bronze_post_item_animation' );
		remove_all_filters( 'bronze_post_columns' );
		remove_all_filters( 'bronze_post_module_layout' );
		if ( 'none' !== $pagination ) {
			...
			do_action( 'bronze_pagination', $query, $pagination_args );
		}
	} else {
		/* No result */
		if ( 'event' === $post_type ) {
			...
			if ( 'past' === $timeline ) {
				...
				apply_filters( 'bronze_no_past_event_text', esc_html__( 'No past event.', 'bronze' ) );

			} else {
				if ( function_exists( 'we_get_option' ) ) {
					echo esc_attr( we_get_option( 'no_event_text', esc_html__( 'No upcoming shows scheduled.', 'bronze' ) ) );
				}
			}
		} elseif ( 'release' === $post_type ) {

			echo esc_attr( apply_filters( 'bronze_no_release_text', esc_html__( 'No release yet.', 'bronze' ) ) );

			if ( is_user_logged_in() ) {
				echo '&nbsp;';
				echo apply_filters( 'bronze_no_release_user_text', esc_html__( 'Is there a featured image set for each of your release?', 'bronze' ) );
			}
		} elseif ( 'video' === $post_type ) {

			echo esc_attr( apply_filters( 'bronze_no_video_text', esc_html__( 'No video yet.', 'bronze' ) ) );

			if ( is_user_logged_in() ) {
				echo '&nbsp;';
				echo esc_attr( apply_filters( 'bronze_no_video_user_text', esc_html__( 'Is there a featured image set for each of your video?', 'bronze' ) ) );
			}
		} elseif ( 'work' === $post_type ) {

			echo esc_attr( apply_filters( 'bronze_no_work_text', esc_html__( 'No work yet.', 'bronze' ) ) );

		} elseif ( 'page' !== $post_type ) {

			get_template_part( bronze_get_template_dirname() . '/components/post/content', 'none' );
		}
	}
	wp_reset_postdata();
}
add_action( 'bronze_posts', 'bronze_output_posts' );
