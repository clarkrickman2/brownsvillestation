 ...
 */
function bronze_get_template_dirname() {
	return apply_filters( 'bronze_template_dirname', 'templates' );
}
 ...
 */
function bronze_get_template_url() {
	return apply_filters( 'bronze_template_folder', bronze_get_template_dirname() );
}
 ...
 */
function bronze_get_hero_background() {

	$hero_bg_type = bronze_get_inherit_mod( 'hero_background_type', apply_filters( 'bronze_default_hero_background_type', 'featured-image' ) );

	if ( is_404() || 'none' === bronze_get_inherit_mod( 'hero_layout' ) || 'none' === $hero_bg_type ) {
		return;
	}

	$post_id = bronze_get_inherit_post_id();
	$overlay = bronze_get_inherit_mod( 'hero_overlay' );
	$output  = '';

	if ( 'image' === $hero_bg_type ) {

		$background_img      = get_post_meta( $post_id, '_post_hero_background_img', true );
		$background_img_url  = bronze_get_url_from_attachment_id( $background_img, 'bronze-XL' );
		...
		$background_effect   = ( get_post_meta( $post_id, '_post_hero_background_effect', true ) ) ? get_post_meta( $post_id, '_post_hero_background_effect', true ) : bronze_get_theme_mod( 'post_header_effect' );
		...
		$output .= bronze_background_img( $img_bg_args );
		...
		$output .= bronze_background_video( $video_bg_args, true );
		...
		$output .= bronze_background_slideshow( $slideshow_args );
		...
		if ( bronze_is_home_as_blog() || 'featured-image' !== $hero_bg_type ) {

			$bg_img_id = bronze_get_hero_image_id();

		} else {
			$bg_img_id = bronze_get_featured_image_id( bronze_get_inherit_post_id(), true );
		}
		...
		$img_bg_args = array(
			...
			'background_effect' => bronze_get_inherit_mod( 'hero_background_effect' ),
		);
	...
	if ( 'none' !== $overlay && $output ) {
		...
		if ( 'custom' === $overlay ) {
			$overlay_color   = bronze_get_inherit_mod( 'hero_overlay_color' );
			$overlay_opacity = bronze_get_inherit_mod( 'hero_overlay_opacity' );
 ...
 * @return string
 */
function bronze_get_header_font_tone() {

	$default_font_tone = ( preg_match( '/dark/', bronze_get_theme_mod( 'color_scheme' ) ) ) ? 'light' : 'dark';
	$header_font_tone  = ( bronze_has_hero() ) ? 'light' : $default_font_tone;
	$header_font_tone  = apply_filters( 'bronze_default_hero_font_tone', $header_font_tone );

	if ( 'none' === bronze_get_inherit_mod( 'hero_layout' ) && 'solid' !== bronze_get_inherit_mod( 'menu_style' ) ) {
		$header_font_tone = 'light';
		$header_font_tone = apply_filters( 'bronze_default_no_header_hero_font_tone', $header_font_tone );
	}
	...
	if ( get_post_meta( bronze_get_inherit_post_id(), '_post_hero_font_tone', true ) ) {
		$header_font_tone = get_post_meta( bronze_get_inherit_post_id(), '_post_hero_font_tone', true );
	}

	return apply_filters( 'bronze_hero_font_tone', $header_font_tone );
}
 ...
 * @return string
 */
function bronze_get_menu_layout() {

	$menu_layout = bronze_get_inherit_mod( 'menu_layout', 'top-right' );

	$post_types = apply_filters( 'bronze_no_header_post_types', array( 'product', 'release', 'event', 'proof_gallery', 'attachment' ) );
	...
	return apply_filters( 'bronze_menu_layout', $menu_layout );
}
 ...
 * @return string
 */
function bronze_get_menu_style() {

	$menu_style = bronze_get_inherit_mod( 'menu_style' );
	...
	if ( is_single() ) {
		...
		if ( ! $post_menu_style_meta && 'none' === bronze_get_inherit_mod( $post_type . '_hero_layout' ) ) {
			$menu_style = 'solid';
		}
	}

	return apply_filters( 'bronze_menu_style', $menu_style );
}

/**
 * Get type of content in the menu call to action area (shop icons, social icons etc...)
 */
function bronze_get_menu_cta_content_type() {

	$content_type = bronze_get_inherit_mod( 'menu_cta_content_type', 'none' );

	return apply_filters( 'bronze_menu_cta_content_type', $content_type );
}
 ...
 */
function bronze_get_single_post_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout      = bronze_get_theme_mod( 'post_single_layout' );
	$single_post_layout_meta = get_post_meta( $post_id, '_post_layout', true );

	if ( is_singular( 'artist' ) ) {
		$single_post_layout = bronze_get_theme_mod( 'artist_single_layout' );
	}
	...
	if ( apply_filters( 'bronze_force_fullwidth_wvc_single_post', true ) && is_singular( 'post' ) && bronze_is_vc() && $single_post_layout_meta !== 'sidebar-left' && $single_post_layout_meta !== 'sidebar-right' ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'bronze_single_post_layout', $single_post_layout );
}

/**
 * Single post WVC layout
 */
function bronze_get_single_post_wvc_layout( $post_id = null ) {
	...
	if ( is_singular( 'post' ) ) {
		...
		if ( bronze_is_vc() ) {
			...
			return apply_filters( 'bronze_single_post_wvc_layout', $single_post_wvc_layout );
		}
	}
}
 ...
 * @return string $layout
 */
function bronze_get_single_video_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = bronze_get_theme_mod( 'video_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'video' ) && bronze_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'bronze_single_video_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function bronze_get_single_mp_event_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = bronze_get_theme_mod( 'mp_event_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'mp_event' ) && bronze_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'bronze_single_mp_event_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function bronze_get_single_mp_column_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = bronze_get_theme_mod( 'mp_column_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'mp_column' ) && bronze_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'bronze_single_mp_column_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function bronze_get_single_artist_layout( $post_id = null, $default = 'default' ) {
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'artist' ) && bronze_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'bronze_single_artist_layout', $single_post_layout );
}
 ...
 * @return string URL
 */
function bronze_get_hero_image_id() {
 ...
 * @return int
 */
function bronze_get_featured_image_id( $post_id = null, $fallback = false ) {
	...
	if ( bronze_get_hero_image_id() && $fallback ) {
		return bronze_get_hero_image_id();
	}
}
 ...
 */
function bronze_get_gallery_params() {

	$array = array();

	if ( bronze_get_post_gallery_ids() ) {

		$bronze_get_post_gallery_ids = bronze_list_to_array( bronze_get_post_gallery_ids() );

		foreach ( $bronze_get_post_gallery_ids as $attachment_id ) {

			$attachment = get_post( $attachment_id );

			if ( $attachment ) {
				$src     = esc_url( bronze_get_url_from_attachment_id( $attachment_id, 'bronze-XL' ) );
 ...
 */
function bronze_get_first_category( $post_id = null ) {
 ...
 * @param int $post_id
 */
function bronze_get_first_category_url( $post_id = null ) {
 ...
 * @return string URL
 */
function bronze_get_hero_image( $args ) {
	...
	$post_id = ( $in_loop ) ? get_the_ID() : bronze_get_inherit_post_id();
 ...
 * @return string URL
 */
function bronze_get_hero_image_src( $in_loop = true ) {
 ...
 * @return string
 */
function bronze_get_first_url( $post_id = null ) {

	if ( is_search() || bronze_is_woocommerce_page() ) {
		return;
	}
 ...
 * @return string
 */
function bronze_get_first_video_url( $post_id = null ) {
 ...
 * @return string
 */
function bronze_get_first_mp3_url( $post_id = null ) {
 ...
 */
function bronze_get_first_quote( $post_id = null ) {
 ...
 * @return string $link
 */
function bronze_remove_more_jump_link( $link ) {
...
add_filter( 'the_content_more_link', 'bronze_remove_more_jump_link' );

/**
 * Get blog index ID
 */
function bronze_get_blog_index_id() {
 ...
 * Get events page_id
 */
function bronze_get_portfolio_page_id() {
 ...
 * Get portfolio URL
 */
function bronze_get_portfolio_url() {
	if ( function_exists( 'wolf_portfolio_get_page_id' ) ) {
 ...
 * Get events page_id
 */
function bronze_get_events_page_id() {
 ...
 * Get events URL
 */
function bronze_get_events_url() {
	if ( function_exists( 'wolf_events_get_page_id' ) ) {
 ...
 * Get albums URL
 */
function bronze_get_albums_url() {
 ...
 */
function bronze_get_discography_page_id() {
 ...
 * Get discography URL
 */
function bronze_get_discography_url() {
	if ( function_exists( 'wolf_discography_get_page_id' ) ) {
 ...
 * Get videos page_id
 */
function bronze_get_videos_page_id() {
 ...
 * Get videos URL
 */
function bronze_get_videos_url() {
	if ( function_exists( 'wolf_videos_get_page_id' ) ) {
 ...
 * Get artists URL
 */
function bronze_get_artists_url() {
 ...
 * Get artists page_id
 */
function bronze_get_artists_page_id() {
 ...
 * Get post type name
 */
function bronze_get_post_type_name() {
 ...
 * Get placeholder URL
 */
function bronze_get_placeholder_url() {
	$rand = rand( 1, 1084 );
 ...
 */
function bronze_get_instagram_image_url( $post_id = null ) {

	if ( ! $post_id ) {
		$post_id = get_the_ID();
	}

	$remote_url = 'https://api.instagram.com/oembed/?url=' . bronze_get_first_url( $post_id );
 ...
 */
function bronze_get_instagram_image( $post_id = null, $link = true ) {
	...
	if ( ! is_wp_error( $response ) && is_array( $response ) ) {
		...
		if ( $thumbnail_url ) {
			return '<a class="instagram-image" href="' . esc_url( bronze_get_first_url( $post_id ) ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '" target="_blank">
 ...
 */
function bronze_get_post_gallery_ids() {
	...
	if ( preg_match( '/images="[0-9 ,]+"/', $content, $match ) ) {
		...
		if ( $gallery && isset( $gallery['ids'] ) ) {
			...
			$ids = bronze_array_to_list( $ids_array );
		}
	}
 ...
 * @return int
 */
function bronze_get_first_gallery_image_count() {

	if ( bronze_get_post_gallery_ids() ) {
		return absint( count( bronze_list_to_array( bronze_get_post_gallery_ids() ) ) );
	}
}
 ...
 */
function bronze_oembed_add_args( $provider, $url, $args ) {
	...
	if ( strpos( $provider, 'vimeo.com' ) ) {
		$provider = add_query_arg(
			array(
				...
				'badge'    => '0',
				'byline'   => '0',
				'color'    => str_replace( '#', '', apply_filters( 'bronze_vimeo_accent_color', bronze_get_inherit_mod( 'accent_color' ) ) ),
			),
			$provider
		);
	}
...
add_filter( 'oembed_fetch_url', 'bronze_oembed_add_args', 10, 3 );
 ...
 * @return bool $bool
 */
function bronze_parallax_mobile( $bool ) {

	if ( bronze_get_theme_mod( 'enable_mobile_parallax' ) ) {
		$bool = false;
	}

	return $bool;
}
add_filter( 'bronze_parallax_no_ios', 'bronze_parallax_mobile', 40 );
add_filter( 'bronze_parallax_no_android', 'bronze_parallax_mobile', 40 );
add_filter( 'bronze_parallax_no_small_screen', 'bronze_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_ios', 'bronze_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_android', 'bronze_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_small_screen', 'bronze_parallax_mobile', 40 );
 ...
 * @return bool $bool
 */
function bronze_animation_mobile( $bool ) {

	if ( bronze_get_theme_mod( 'enable_mobile_animations' ) ) {
		$bool = true;
	}

	return $bool;
}
add_filter( 'bronze_force_animation_mobile', 'bronze_animation_mobile', 40 );
add_filter( 'wvc_force_animation_mobile', 'bronze_animation_mobile', 40 );
 ...
 */
function bronze_overwrite_lightbox( $lightbox ) {
	return 'fancybox';
}
add_filter( 'wvc_lightbox', 'bronze_overwrite_lightbox', 40 );

/**
 * Site footer class
 */
function bronze_set_site_footer_tone_class( $class ) {

	$body_bg_color = bronze_get_theme_mod( 'body_background_color' );
...
add_filter( 'bronze_site_footer_class', 'bronze_set_site_footer_tone_class' );
