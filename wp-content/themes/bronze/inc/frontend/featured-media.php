<?php
/**
 * Bronze featured media function
 *
 * @package WordPress
 * @subpackage Bronze
 * @version 1.1.0
 ...
 */
function bronze_featured_media( $post_id = null ) {
	$output = '';
	...
	if ( $has_post_thumbnail ) {
		...
		$default_thumbnail .= '<div class="entry-thumbnail">';
			$default_thumbnail .= '<a href="' . esc_url( $permalink ) . '" class="entry-link" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'bronze' ), the_title_attribute( 'echo=0' ) ) ) . '">';
	...
	if ( 'standard' === $post_format || 'chat' === $post_format ) {
		...
		if ( bronze_is_instagram_post( $post_id ) ) {

			/**
			 * Embed Instagram
			 */
			$output .= bronze_get_instagram_image( $post_id );
			 ...
			 * Featured image
			 */
			$large_img_src = get_the_post_thumbnail_url( '', 'bronze-XL' );
		...
		if ( bronze_entry_slider() ) {

			$output .= bronze_entry_slider();
		...
		if ( ! bronze_is_playlist_audio_player( $post_id ) ) {
			$output .= $default_thumbnail;
		}

		/* Highlight the audio file. */
		if ( bronze_get_audio_embed_content( $post_id ) ) {
			$output .= bronze_get_audio_embed_content( $post_id );
		}

	} elseif ( 'quote' === $post_format ) {

		if ( bronze_get_first_quote( $post_id ) ) {
			$output .= '<div class="entry-thumbnail">';
				$output .= '<a href="' . esc_url( $permalink ) . '" class="entry-link-mask" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'bronze' ), the_title_attribute( 'echo=0' ) ) ) . '"></a>';
				$output .= bronze_background_img();

				$output .= '<div class="entry-summary-overlay">';
					$output .= '<blockquote class="entry-featured-quote">';
						$output .= bronze_get_first_quote( $post_id );
		...
		if ( bronze_get_first_url( $post_id ) ) {
			$output .= '<div class="entry-thumbnail">';
				$output .= '<a href="' . esc_url( bronze_get_first_url( $post_id ) ) . '" target="_blank" class="entry-link-mask"></a>';
				$output .= bronze_background_img();
				$output .= '<div class="entry-summary-overlay">';
			...
			$output .= '<div class="entry-thumbnail">';
				...
				$output .= '<div class="entry-summary-overlay">';
					$output .= '<div class="entry-featured-status">';
						$output .= bronze_sample( get_the_content(), 33 );
