...
if ( ! class_exists( 'Bronze_Walker_Comment' ) ) {
	class Bronze_Walker_Comment extends Walker_Comment {
			...
			<article <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>" itemscope itemtype="http://schema.org/Comment">
				<div class="comment-content" itemprop="text">
					...
					<div class="comment-meta post-meta" role="complementary">
						...
						<?php edit_comment_link( '<p class="comment-meta-item">' . esc_html__( 'Edit this comment', 'bronze' ) . '</p>', '', '' ); ?>
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<p class="comment-meta-item"><?php esc_html_e( 'Your comment is awaiting moderation.', 'bronze' ); ?></p>
						<?php endif; ?>
						<?php comment_text(); ?>
						<?php comment_reply_link(
								array_merge(
									$args,
									[
										'reply_text' => '<span>' . esc_html__( 'Reply', 'bronze' ) . '</span>',
										'add_below' => $add_below,
										'depth' => $depth,
