 ...
 * @return string
 */
function bronze_convert_img_size_name( $name = 'standard' ) {
 ...
 * @return string
 */
function bronze_convert_img_dimension_percent_ratio( $dimensions ) {
 ...
 * @return void
 */
function bronze_resized_thumbnail( $size = '150x150', $class = '', $image_id = null, $echo = true ) {
	...
	if ( $echo ) {
		echo bronze_kses( $thumbnail['thumbnail'] );
	}

	return bronze_kses( $thumbnail['thumbnail'] );
}
 ...
 * @return string
 */
function bronze_filter_post_module_thumbnail_size( $custom_thumbnail_size, $display, $atts ) {
...
add_filter( 'bronze_post_module_custom_thumbnail_size', 'bronze_filter_post_module_thumbnail_size', 3, 10 );
 ...
 * @return void
 */
function bronze_optimize_thumbnail_size( $atts ) {
	...
	if ( 'column' === $column_type ) {
		if ( 'full' === $content_width || 'large' === $content_width ) {
			add_filter( 'bronze_thumbnail_size_name', 'bronze_set_large_thumbnail_size' );
		}
	}
}
 ...
 */
function bronze_get_img_by_size( $params = array() ) {
	$params = array_merge(
		array(
			'post_id'    => null,
			'attach_id'  => null,
	...
	$attach_id   = apply_filters( 'bronze_object_id', $attach_id );
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, $sizes, true ) ) ) {
		...
		if ( is_array( $thumb_size ) ) {
			$p_img      = bronze_resize_image( $attach_id, null, $thumb_size[0], $thumb_size[1], true );
			...
			if ( ! empty( $attachment ) ) {
				...
				if ( $p_img ) {
					...
					$attributes = bronze_stringify_attributes(
						array(
							'class'  => $thumb_class,
							'src'    => $p_img['url'],
							'width'  => $p_img['width'],
...
if ( ! function_exists( 'bronze_resize_image' ) ) {
	/**
	 ...
	 * @since 4.2
	 */
	function bronze_resize_image( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
		$image_src = array();
		if ( $attach_id ) {
 ...
 * @return string
 */
function bronze_stringify_attributes( $attributes ) {
	$atts = array();
