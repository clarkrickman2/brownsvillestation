<?php
/**
 * Bronze Customizer library class
 *
...
if ( ! class_exists( 'Bronze_Customizer_Library' ) ) {
	/**
	 * Bronze Customizer library class
	 *
	 * Create customizer inputs from array
	 ...
	 * @version 1.1.0
	 */
	class Bronze_Customizer_Library {
		...
		public function register_sections( $wp_customize ) {
			...
			foreach ( $this->sections as $section ) {
				...
				$title         = isset( $section['title'] ) ? $section['title'] : esc_html__( 'Section Title', 'bronze' );
				...
				if ( $is_background ) {
					...
					foreach ( $options as $option ) {
						...
						if ( 'text' == $type ) {
							...
							$wp_customize->add_control(
								$bronze_wp_customize_color_control = new WP_Customize_Color_Control(
									$wp_customize,
									$option_id,
									array(
										'label'       => $label,
							...
							$wp_customize->add_control(
								$bronze_wp_customize_image_control = new WP_Customize_Image_Control(
									$wp_customize,
									$option_id,
									array(
										'label'       => $label,
							...
							$wp_customize->add_control(
								$bronze_wp_customize_image_control = new WP_Customize_Media_Control(
									$wp_customize,
									$option_id,
									array(
										'label'       => $label,
							...
							$wp_customize->add_control(
								$bronze_customize_textarea_control = new Wolftheme_Customize_Textarea_Control(
									$wp_customize,
									$option_id,
									array(
										'label'    => $label,
							...
							$wp_customize->add_control(
								$bronze_customize_textarea_control = new WP_Customize_Radio_Images_Control(
									$wp_customize,
									$option_id,
									array(
										'label'    => $label,
							...
							$wp_customize->add_control(
								$bronze_customize_textarea_control = new WP_Customize_Group_Checkbox_Control(
									$wp_customize,
									$option_id,
									array(
										'label'    => $label,
		...
		public function background_setting( $option, $section_id, $wp_customize, $section = true, $priority = 35 ) {
			...
			if ( $font_color ) {
				...
				$wp_customize->add_control(
					$bronze_wp_customize_color_control = new WP_Customize_Color_Control(
						...
						$background_id . '_font_color',
						array(
							'label'    => esc_html__( 'Font Color', 'bronze' ),
							'section'  => $section_id,
			...
			if ( $is_no_bg ) {
				...
				$wp_customize->add_control(
					...
					array(
						'type'     => 'checkbox',
						'label'    => esc_html__( 'No Background', 'bronze' ),
						'section'  => $section_id,
						'priority' => 1,
					)
				);
			...
			if ( $opacity ) :
				...
				$wp_customize->add_control(
					$background_id . '_opacity',
					array(
						'label'    => esc_html__( 'Background Color Opacity (in percent)', 'bronze' ),
						'section'  => $section_id,
						'type'     => 'text',
						'priority' => 3,
					)
			...
			if ( $is_bg_img ) :
				...
				$wp_customize->add_control(
					$bronze_wp_customize_image_control = new WP_Customize_Image_Control(
						...
						$background_id . '_img',
						array(
							'label'             => esc_html__( 'Background Image', 'bronze' ),
							'section'           => $section_id,
				...
				$wp_customize->add_control(
					...
					array(
						'type'     => 'select',
						'label'    => esc_html__( 'Background Repeat', 'bronze' ),
						'section'  => $section_id,
						'choices'  => array(
							'no-repeat' => 'no-repeat',
							'repeat'    => 'repeat',
				...
				$wp_customize->add_control(
					...
					array(
						'type'     => 'select',
						'label'    => esc_html__( 'Background Position', 'bronze' ),
						'section'  => $section_id,
						'choices'  => array(
				...
				$wp_customize->add_control(
					...
					array(
						'type'     => 'select',
						'label'    => esc_html__( 'Background Attachment', 'bronze' ),
						'section'  => $section_id,
						'choices'  => array(
							'scroll' => 'scroll',
							'fixed'  => 'fixed',
				...
				$wp_customize->add_control(
					...
					array(
						'type'     => 'select',
						'label'    => esc_html__( 'Background Size', 'bronze' ),
						'section'  => $section_id,
						'choices'  => array(
							'cover'     => esc_html__( 'Cover', 'bronze' ),
							'contain'   => esc_html__( 'Contain', 'bronze' ),
							'100% auto' => esc_html__( '100% width', 'bronze' ),
							'auto 100%' => esc_html__( '100% height', 'bronze' ),
							'inherit'   => esc_html__( 'Inherit', 'bronze' ),
						),
						'priority' => 8,
					)
				);
				...
				if ( $parallax ) {
					...
					$wp_customize->add_control(
						...
						array(
							'type'     => 'checkbox',
							'label'    => esc_html__( 'Parallax', 'bronze' ),
							'section'  => $section_id,
							'priority' => 9,
						)
					);
		...
		public function sanitize_group_checkbox( $input ) {
			...
			return bronze_clean_spaces( $input );
		}
...
if ( class_exists( 'WP_Customize_Control' ) ) :
	class WP_Customize_Group_Checkbox_Control extends WP_Customize_Control {
		...
		public function enqueue() {
			wp_enqueue_script( 'bronze-customize-controls', get_template_directory_uri() . '/inc/customizer/js/customize-controls.js', array( 'jquery' ) );
		}
		...
		public function render_content() {
			...
			$multi_values = bronze_list_to_array( $this->value() );
			?>
	...
	class WP_Customize_Radio_Images_Control extends WP_Customize_Control { // phpcs:ignore
		...
		public function render_content() {
			...
			<?php foreach ( $this->choices as $choice ) : ?>
				<label class="bronze-radio-image-label" style="width:<?php echo absint( $itemwidth ); ?>%;">
					...
					<img class="bronze-radio-image" src="<?php echo esc_url( $choice['image'] ); ?>">
					<span class="bronze-radio-image-text"><?php echo esc_attr( $choice['text'] ); ?></span>
				</label>
			<?php endforeach; ?>
			<?php
		}
