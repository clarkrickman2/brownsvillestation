<?php
/**
 * Bronze customizer colors preview functions
 *
 * @package WordPress
 * @subpackage Bronze
 * @version 1.1.0
...
/**
 * Registers color schemes for Bronze.
 *
 * Can be filtered with {@see 'bronze_color_schemes'}.
 *
 ...
 */
function bronze_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Bronze.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'bronze_color_schemes', [

		'default' => [
			'label'  => esc_html__( 'Default', 'bronze' ),
			'colors' => [
				'#ffffff', // body_bg
				'#ffffff', // page_bg
 ...
 */
function bronze_get_color_scheme_css( $colors ) {
	...
	$border_color = vsprintf( 'rgba( %s, 0.08)', bronze_hex_to_rgb( $strong_text_color ) );
	...
	***********************/
	...
	return apply_filters( 'bronze_color_scheme_output', $output, $colors );
}
 ...
 * @return array $colors
 */
function bronze_get_template_colors() {
 ...
 * Customizer preview.
 */
function bronze_color_scheme_css_template() {

	$colors = bronze_get_template_colors();
	?>
	<script type="text/html" id="tmpl-bronze-color-scheme">
		<?php echo bronze_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'bronze_color_scheme_css_template' );
