 ...
 */
function bronze_get_fonts_css( $fonts_args ) {
	...
	/* Body */
	$body_selectors = apply_filters( 'bronze_body_font_family_selectors', bronze_list_to_array( 'body, blockquote.wvc-testimonial-content, .tp-caption:not(h1):not(h2):not(h3):not(h4):not(h5)' ) );

	$body_selectors = bronze_array_to_list( $body_selectors );
	...
	$menu_selectors = apply_filters( 'bronze_menu_selectors', bronze_list_to_array( '.nav-menu li, .cta-container' ) );

	$menu_selectors = bronze_array_to_list( $menu_selectors );
	...
	/* Heading */
	$heading_family_selectors = apply_filters( 'bronze_heading_family_selectors', bronze_list_to_array( 'h1, h2, h3, h4, h5, h6, .post-title, .entry-title, h2.entry-title > .entry-link, h2.entry-title, .widget-title, .wvc-counter-text, .wvc-countdown-period, .event-date, .logo-text, .wvc-interactive-links, .wvc-interactive-overlays, .heading-font' ) );
	...
	$heading_family_selectors = bronze_array_to_list( $heading_family_selectors );
	...
	return apply_filters( 'bronze_fonts_css_output', $font_css, $fonts_args );
}
 ...
 * @return array $fonts
 */
function bronze_get_template_fonts() {
 ...
 * Customizer preview.
 */
function bronze_fonts_css_template() {

	$fonts = bronze_get_template_fonts();
	?>
	<script type="text/html" id="tmpl-bronze-fonts">
		<?php echo bronze_get_fonts_css( $fonts ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'bronze_fonts_css_template' );
