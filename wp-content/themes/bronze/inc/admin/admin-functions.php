 ...
 */
function bronze_server_requirements_notice() {

	global $pagenow;

	$requirements_ok = true;

	foreach ( bronze_get_minimum_required_server_vars() as $constant => $required_value ) {
	...
	if ( ! $requirements_ok && 'index.php' === $pagenow ) {
		$message = sprintf(
			...
			bronze_kses( __( 'It is recommended that your server settings match the recommended minimum requirements to run %1$s smoothly. Please review your <a href="%2$s">system status</a>.', 'bronze' ) ),
			bronze_get_theme_name(),
			esc_url( admin_url( 'themes.php?page=' . bronze_get_theme_slug() . '-about#system-status' ) )
		);
		bronze_admin_notice( $message, 'info', bronze_get_theme_slug() . '_server_requirements', esc_html__( 'Dismiss this notice', 'bronze' ) );
	}
}
add_action( 'admin_init', 'bronze_server_requirements_notice' );
 ...
 */
function bronze_add_excerpt_support_for_post_types() {
	add_post_type_support( 'work', 'excerpt' );
	add_post_type_support( 'release', 'excerpt' );
}
add_action( 'init', 'bronze_add_excerpt_support_for_post_types' );

/**
 * Add helper admin notice on work post
 */
function bronze_help_admin_notice() {
	...
	if ( ! $import_demo_flag && ! $dissmissed_import_demo_flag && ! $is_ocdi_page && class_exists( 'OCDI_Plugin' ) && 'index.php' === $pagenow ) {
		...
		$message  = '<h3>';
		$message .= esc_html__( 'Hey there, would you like to import the demo content to help you to get started?', 'bronze' );
		$message .= '</h3>';

		$message .= '<h4>';
		$message .= esc_html__( 'In this case, please ignore the plugin page setups below.', 'bronze' );
		$message .= '</h4>';

		$message .= '<p>';
		$message .= sprintf(
			bronze_kses( __( 'If you dismiss this notification, you can still access the demo import page via the "Appearance > <a href="%s">Import Demo Data</a>" panel.', 'bronze' ) ),
		...
		$message .= sprintf(
			...
			bronze_kses( __( '<a href="%s" class="button button-primary button-hero">Install demo data</a><br>', 'bronze' ) ),
			esc_url( admin_url( 'themes.php?page=pt-one-click-demo-import' ) )
		);
		...
		$message .= sprintf(
			...
			bronze_kses( __( '<a href="%s">No thanks</a><br>', 'bronze' ) ),
			esc_url( admin_url( 'index.php?' . $theme_slug . '-dismiss-demo-data-import=yes' ) )
		);
		$message .= '</p>';

		bronze_admin_notice( $message, 'info' );
	}
	...
	if ( $import_demo_flag && class_exists( 'WooCommerce' ) && 'index.php' === $pagenow ) {
		...
		$message = esc_html__( 'The demo data was successfully imported. ', 'bronze' );

		$message .= sprintf(
			/* translators: %s: install demo import notification URL */
			bronze_kses( __( '<a href="%1$s" class="button button-primary button-hero">Install demo data</a><br>', 'bronze' ) ),
	...
	if ( 'work' === $post_type && ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) ) {
		$message   = esc_html__( 'Please use the main text editor to showcase your media content and the "excerpt" box to insert an explanatory text in the page.', 'bronze' );
		$cookie_id = $theme_slug . '_wolf_work_help';

		bronze_admin_notice( $message, 'info', $cookie_id, esc_html__( 'Dismiss this notice', 'bronze' ) );
	}

	/* Release */
	if ( 'release' === $post_type && ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) ) {
		$message   = esc_html__( 'You can use the main text editor to showcase your media content usign the page builder, with a playlist for example. In this case it is recommended to set your row "Content Width" to "Full Width" and the background settings to "No Background".', 'bronze' );
	...
	if ( ( 'mp-event' === $post_type || 'mp-colum' === $post_type ) && ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) ) {
		...
		$message   = esc_html__( 'If your post content is designed with the page builder, it is recommended to set your row "Content Width" to "Full Width" and the background settings to "No Background".', 'bronze' );
	...
	if ( 'post' === $post_type && ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) ) {
		...
		if ( bronze_has_vc_content() ) {
			$message   = esc_html__( 'If your post content is designed with the page builder, it is recommended to enter a post text sample in the "excerpt" box.', 'bronze' );
			$message  .= ' ' . esc_html__( 'In this case it is recommended to set your row "Content Width" to "Full Width" and the background settings to "No Background".', 'bronze' );
	...
	if ( ! get_option( 'wpb_js_gutenberg_disable' ) && defined( 'WPB_VC_VERSION' ) && 'index.php' === $pagenow && ! defined( 'DOING_AJAX' ) ) {
		$message = sprintf(
			...
			bronze_kses( __( 'It seems that both <strong>%1$s</strong> and <strong>%2$s</strong> are enabled. You can disable %2$s in the <a href="%3$s">%4$s</a>.<br>', 'bronze' ) ),
		...
		bronze_admin_notice( $message, 'warning', $cookie_id, esc_html__( 'Dismiss this notice', 'bronze' ) );
	}
}
add_action( 'admin_init', 'bronze_help_admin_notice' );
 ...
 */
function bronze_admin_notice( $message = null, $type = null, $cookie_id = null, $dismiss_text = null ) {
	...
	if ( $cookie_id ) {
		...
		if ( ! $dismiss_text ) {
			$dismiss_text = esc_html__( 'Hide permanently', 'bronze' );
		}

		if ( $cookie_id ) {
			if ( ! isset( $_COOKIE[ $cookie_id ] ) ) {
				$href = esc_url( admin_url( 'themes.php?page=' . bronze_get_theme_slug() . '-about&amp;dismiss=' . $cookie_id ) );
				echo "<div class='notice notice-$type $is_dismissible'><p>$message</p><p><a href='$href' id='$cookie_id' class='button bronze-dismiss-admin-notice'>$dismiss_text</a></p></div>"; // WCS XSS ok.
			}
		}
	} else {
...
add_action( 'admin_notices', 'bronze_admin_notice' );

/**
 * Remove post formats on work posts
 */
function bronze_remove_work_post_formats() {

	if ( apply_filters( 'bronze_disable_work_post_formats', true ) ) {
...
add_action( 'load-post.php', 'bronze_remove_work_post_formats' );
add_action( 'load-post-new.php', 'bronze_remove_work_post_formats' );

/**
 * Remove unwanted plugin submenu
 */
function bronze_remove_wolf_plugin_submenu() {
...
add_action( 'admin_menu', 'bronze_remove_wolf_plugin_submenu', 999 );
 ...
 */
function bronze_sync_theme_font_options_with_wvc( $options ) {
...
add_action( 'bronze_after_' . bronze_get_theme_slug() . '_font_settings_options_save', 'bronze_sync_theme_font_options_with_wvc', 10, 1 );
 ...
 * @return array $options
 */
function bronze_sync_wvc_font_options_with_theme( $options ) {

	if ( isset( $options['google_fonts'] ) ) {
		$fonts = $options['google_fonts'];
		bronze_update_option( 'font', 'google_fonts', $fonts );
	}

	return $options;
}
add_action( 'wvc_after_options_save', 'bronze_sync_wvc_font_options_with_theme', 10, 1 );
add_action( 'wolf_core_after_options_save', 'bronze_sync_wvc_font_options_with_theme', 10, 1 );
 ...
 */
function bronze_sync_theme_social_mods_with_page_builder() {
...
add_action( 'customize_save_after', 'bronze_sync_theme_social_mods_with_page_builder' );
 ...
 * @return array $options
 */
function bronze_sync_wvc_social_options_with_theme( $options ) {
	if ( function_exists( 'wvc_get_socials' ) ) {
		$wvc_socials = wvc_get_socials();
...
add_action( 'wvc_after_options_save', 'bronze_sync_wvc_social_options_with_theme' );
 ...
 * @return array $options
 */
function bronze_add_cta_menu_content_types( $options ) {

	if ( bronze_is_wvc_activated() ) {
		$options['socials'] = esc_html__( 'Socials', 'bronze' );
	}

	if ( bronze_is_wc_activated() ) {
		$options['shop_icons'] = esc_html__( 'Shop Icons', 'bronze' );
	}

	if ( function_exists( 'icl_object_id' ) ) {
		$options['wpml'] = esc_html__( 'Language Switcher', 'bronze' );
	}

	return $options;
}
add_filter( 'bronze_menu_cta_content_type_options', 'bronze_add_cta_menu_content_types' );
 ...
 * @return string $mod
 */
function bronze_filter_menu_cta_content_type( $mod ) {

	if ( 'socials' === $mod && ! bronze_is_wvc_activated() ) {
		$mod = 'icons';
	}

	if ( 'wpml' === $mod && ! bronze_is_wvc_activated() ) {
		$mod = 'icons';
	}

	return $mod;
}
add_filter( 'theme_mod_cta_content', 'bronze_filter_menu_layout_theme_mods' );
 ...
 * @return bool
 */
function bronze_has_vc_content() {
 ...
 * @return array $result
 */
function bronze_get_revsliders() {
	...
	if ( class_exists( 'RevSlider' ) ) {
		...
		if ( $alias_array && $title_array ) {
			...
			$result = array( '' => esc_html__( 'No slider yet', 'bronze' ) );
		}
 ...
 * @return array
 */
function bronze_mce_styleselect_button( $buttons ) {
	array_unshift( $buttons, 'styleselect' );
	return $buttons;
}
add_filter( 'mce_buttons_2', 'bronze_mce_styleselect_button' );
 ...
 * @return array
 */
function bronze_mce_before_init_insert_formats( $init_array ) {

	$style_formats = array(
		array(
			'title'   => esc_html__( 'Accent Color', 'bronze' ),
			'inline'  => 'span',
			'classes' => 'accent',
			'wrapper' => false,
		),
		...
		array(
			'title'   => esc_html__( 'Caption', 'bronze' ),
			'inline'  => 'span',
			'classes' => 'caption',
			'wrapper' => false,
		),
	...
	$style_formats = apply_filters( 'bronze_tiny_mce_style_formats', $style_formats );
	$init_array['style_formats'] = wp_json_encode( $style_formats );

	return $init_array;
}
add_filter( 'tiny_mce_before_init', 'bronze_mce_before_init_insert_formats' );
