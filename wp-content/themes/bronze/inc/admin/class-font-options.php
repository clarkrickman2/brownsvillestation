...
if ( ! class_exists( 'Bronze_Font_Options' ) ) {
	/**
	 * Bronze font loader
	 *
	...
	class Bronze_Font_Options {
		...
		public function add_menu() {
			...
			add_theme_page( esc_html__( 'Fonts Loader', 'bronze' ), esc_html__( 'Font Loader', 'bronze' ), 'administrator', 'font-loader', array( $this, 'font_loader_settings' ) );
		}
		...
		public function options_index_name() {
			return bronze_get_theme_slug() . '_' . $this->index . '_settings';
		}
		...
		public function admin_init() {
			...
			add_settings_field( 'google_fonts', esc_html__( 'Google Fonts', 'bronze' ), array( $this, 'setting_google_font' ), $this->options_index_name(), $this->options_index_name() );

			add_settings_field( 'typekit_fonts', esc_html__( 'Typekit fonts', 'bronze' ), array( $this, 'setting_typekit_font' ), $this->options_index_name(), $this->options_index_name() );
		}
		...
		public function settings_validate( $input ) {
			...
			if ( isset( $_POST['bronze_font_settings_nonce'] ) && wp_verify_nonce( $_POST['bronze_font_settings_nonce'], 'bronze_save_font_settings_nonce' ) ) {
				...
				do_action( 'bronze_after_' . $this->options_index_name() . '_options_save', $input );
			}
		...
		public function setting_google_font() {
			...
			<p class="description">
				<?php
					echo sprintf(
						bronze_kses(
							__( 'You can get your fonts on the <a href="%s" target="_blank">Google Fonts</a> website.', 'bronze' )
						),
		...
		public function setting_typekit_font() {
			...
			<p class="description">
				<?php
					echo sprintf(
						bronze_kses(
							__( 'You need <a href="%1$s" target="_blank">Typekit Fonts for WordPress</a> plugin to import your font kit into your website. <a href="%2$s" target="_blank">More infos</a>', 'bronze' )
						),
		...
		public function default_options() {
			...
			if ( false === get_option( $this->options_index_name() ) ) {
				...
				$default = array(
					'google_fonts'  => apply_filters( 'bronze_default_google_fonts', '' ),
					'typekit_fonts' => apply_filters( 'bronze_default_typekit_fonts', '' ),
				);
		...
		public function font_loader_settings() {
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Fonts Loader', 'bronze' ); ?></h2>
				<?php if ( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] ) { ?>
					<div id="setting-error-settings_updated" class="updated settings-error">
						<p><strong><?php esc_html_e( 'Settings saved.', 'bronze' ); ?></strong></p>
					</div>
				<?php } ?>
				<form action="options.php" method="post">
					<?php wp_nonce_field( 'bronze_save_font_settings_nonce', 'bronze_font_settings_nonce' ); ?>
					<?php settings_fields( $this->options_index_name() ); ?>
					<?php do_settings_sections( $this->options_index_name() ); ?>
					<p class="submit"><input name="save" type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'bronze' ); ?>" /></p>
				</form>
			</div>
			<?php
		}
