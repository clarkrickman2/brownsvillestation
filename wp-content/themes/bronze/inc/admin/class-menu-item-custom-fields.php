<?php
/**
 * Bronze Menu item custom fields example
 *
 ...
 * @since 0.1.0
 */
class Bronze_Menu_Item_Custom_Fields {
	...
	public static function fields( $item, $depth, $args = array(), $id = 0 ) {
		...
		?>
			...
			<p class="field-_menu-item-button-style description description-wide">
				<label for="edit-_menu-item-button-style-<?php echo esc_attr( $item_id ); ?>">
					<select name="_menu-item-button-style[<?php echo esc_attr( $item_id ); ?>]">
						...
						<option value="primary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'primary' ); ?>><?php esc_html_e( 'Primary', 'bronze' ); ?></option>
						<option value="secondary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'secondary' ); ?>><?php esc_html_e( 'Secondary', 'bronze' ); ?></option>
					</select>
			...
			echo apply_filters( 'bronze_menu_meta_fields_markup', ob_get_clean(), $item_id );
	}
...
Bronze_Menu_Item_Custom_Fields::init();
