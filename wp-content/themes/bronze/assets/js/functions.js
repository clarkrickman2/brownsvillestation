 ...
 */
/* jshint -W062 */
/* global BronzeParams, WOW, AOS, WVC, Cookies, Event, objectFitImages, WVCBigText, WVCParams, BronzeYTVideoBg, Vimeo */
	...
	return {
		...
		init: function () {
			...
			this.mobileScreenBreakpoint =
				BronzeParams.mobileScreenBreakpoint;
			this.tabletScreenBreakpoint =
				BronzeParams.tabletScreenBreakpoint;
			this.notebookScreenBreakpoint =
				BronzeParams.notebookScreenBreakpoint;
			this.desktopScreenBreakpoint =
				BronzeParams.desktopScreenBreakpoint;
			this.desktopBigScreenBreakpoint =
				BronzeParams.desktopBigScreenBreakpoint;
		...
		setClasses: function () {
			...
			if (
				...
				!BronzeParams.forceAnimationMobile
			) {
		...
		loadingAnimation: function () {
			if (!BronzeParams.defaultPageLoadingAnimation) {
				return false;
			}
		...
		mobileMenuBreakPoint: function () {
			var $body = $('body'),
				...
				breakpoint = BronzeParams.breakPoint;
			...
			if (breakpoint > winWidth) {
				...
				$(window).trigger('bronze_breakpoint');
			} else {
		...
		subMenuDirection: function () {
			var $this,
				subMenuWidth = parseInt(BronzeParams.subMenuWidth, 10),
				bleed = 8;
		...
		stickyMenu: function (scrollTop) {
			var scrollPoint,
				menuOffset = parseInt(BronzeParams.menuOffset, 10);

			scrollTop = scrollTop || 0;

			scrollPoint =
				parseInt(BronzeParams.stickyMenuScrollPoint, 10) ||
				$('body').offset().top;
			...
			if (menuOffset) {
				if ('100%' === BronzeParams.menuOffset) {
					scrollPoint =
						$(window).height() -
			...
			if ('soft' === BronzeParams.stickyMenuType) {
			...
			} else if ('hard' === BronzeParams.stickyMenuType) {
				if (scrollPoint < scrollTop) {
		...
		parallax: function () {
			var smallScreen =
				...
				BronzeParams.parallaxNoSmallScreen;

			if (!smallScreen) {
				$('.parallax').jarallax({
					noAndroid: BronzeParams.parallaxNoAndroid,
					noIos: BronzeParams.parallaxNoIos,
				});
			}
		},
		...
		toggleMenu: function () {
			$(document).on('click', '.toggle-mobile-menu', function (event) {
				...
				$(window).trigger('bronze_mobile_menu_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-side-panel', function (event) {
				event.preventDefault();
				$(window).trigger('bronze_side_panel_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-offcanvas-menu', function (event) {
				...
				$(
					window
				).trigger('bronze_offcanvas_menu_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-overlay-menu', function (event) {
				...
				$(window).trigger(
					'bronze_overlay_menu_toggle_button_click',
					[$(this)]
				);
			});
			...
			$(document).on('click', '.toggle-lateral-menu', function (event) {
				event.preventDefault();
				$(window).trigger(
					'bronze_lateral_menu_toggle_button_click',
					[$(this)]
				);
		...
		toggleSearchForm: function () {
			$(document).on('click', '.toggle-search', function () {
				$(window).trigger('bronze_searchform_toggle');
		...
		flexSlider: function () {
			if ($.isFunction($.flexslider)) {
				...
				$('.entry-slider').flexslider({
					animation: BronzeParams.entrySliderAnimation,
					slideshow: true,
					slideshowSpeed: 4000,
				});
			}
		...
		lightbox: function () {
			...
			if ('fancybox' === BronzeParams.lightbox) {
				$(selectors).fancybox(BronzeParams.fancyboxSettings);
				...
				$('.gallery-quickview, .wvc-gallery-quickview')
					...
					.on('click', function () {
						...
						params = BronzeParams.fancyboxSettings;
				...
				$('.lightbox-video').fancybox({
					beforeLoad: function () {
						BronzeUi.pausePlayers();
					},

					afterLoad: function () {
						BronzeUi.lightboxVideoAfterLoad();
					},

					afterClose: function () {
						BronzeUi.restartVideoBackgrounds();
					},
				});
			}
		},
		...
		lightboxVideoAfterLoad: function () {
			var $iframe = $('.fancybox-iframe').contents(),
				...
				accentColor = BronzeParams.accentColor;
			...
			if ($video.length) {
				...
				$head.append(
					$('<link/>', {
						rel: 'stylesheet',
						href: BronzeParams.mediaelementLegacyCssUri,
						type: 'text/css',
					})
				);
				$head.append(
					$('<link/>', {
						rel: 'stylesheet',
						href: BronzeParams.mediaelementCssUri,
						type: 'text/css',
					})
				);
		...
		scrollToMainContent: function () {
			...
			$('html, body')
				...
				.animate(
					...
					parseInt(BronzeParams.smoothScrollSpeed, 10),
					BronzeParams.smoothScrollEase,
					function () {
						if ('' !== hash) {
		...
		wowAnimate: function () {
			...
			if (doWow && 'undefined' !== typeof WOW) {
				wowAnimate = new WOW({
					offset: BronzeParams.WOWAnimationOffset,
		...
		setInternalLinkClass: function () {
			var siteURL = BronzeParams.siteUrl,
				$internalLinks,
				regEx = '';

			$.each(BronzeParams.allowedMimeTypes, function (
				index,
				value
			) {
				regEx += '|' + value;
			...
			if (BronzeParams.isWooCommerce) {
				...
				if (
					...
					this.untrailingSlashit(
						BronzeParams.WooCommerceCartUrl
					)
				) {
					$(
						'a[href^="' +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						BronzeParams.WooCommerceAccountUrl
					)
				) {
					$(
						'a[href^="' +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						BronzeParams.WooCommerceCheckoutUrl
					)
				) {
					$(
						'a[href^="' +
		...
		transitionCosmetic: function () {
			if (!BronzeParams.defaultPageTransitionAnimation) {
				return false;
			}

			var _this = this;

			if (BronzeParams.isAjaxNav) {
				return;
			}
			...
			$(document).on('click', '.internal-link:not(.disabled)', function (
				...
				if (!event.ctrlKey) {
					...
					Cookies.set(
						BronzeParams.themeSlug + '_session_loaded',
						true,
						{
							expires: null,
						}
					...
					if (BronzeParams.hasLoadingOverlay) {
						$('.loading-overlay').one(
							_this.transitionEventEnd(),
							function () {
								Cookies.remove(
									BronzeParams.themeSlug +
										'_session_loaded'
								);
		...
		videoThumbnailPlayOnHover: function () {
			...
			$('iframe.youtube-bg', itemsContainer).each(function () {
				if (
					'undefined' !== typeof BronzeYTVideoBg &&
					BronzeYTVideoBg.players[
				...
				if ($iframe.length) {
					if ($iframe.hasClass('youtube-bg')) {
						$(this).on('mouseenter', function () {
							if (
								'undefined' !== typeof BronzeYTVideoBg &&
								BronzeYTVideoBg.players[YTPlayerId]
							) {
								BronzeYTVideoBg.players[
									YTPlayerId
								].playVideo();
		...
		navPlayer: function () {
			$(document).on('click', '.nav-play-button', function () {
				...
				var $btn = $(this),
					...
					audioId = $audio.attr('id'),
					audio = document.getElementById(audioId),
					playText = BronzeParams.l10n.playText,
					pauseText = BronzeParams.l10n.pauseText;
		...
		pageLoad: function () {
			...
			delay = BronzeParams.pageTransitionDelayAfter;
			...
			}, parseInt(BronzeParams.pageLoadedDelay, 10));
		},
	};
})(jQuery);
...
(function ($) {
	...
	if ('undefined' !== typeof WVC) {
		 ...
		 */
		WVC.lightbox = BronzeUi.lightbox;

		/**
		 * Overwrite WVC video opener
		 */
		WVC.videoOpener = BronzeUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WVC.getToolBarOffset = BronzeUi.getToolBarOffset;
	}
