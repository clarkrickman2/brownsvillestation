 ...
 */
/* jshint -W062 */
/* global Event, BronzeUi, BronzeParams */
var BronzeCarousels = function( $ ) {
	...
	return {
		...
		moduleCarousel : function () {
			...
			if ( BronzeUi.isMobile ) {
				return;
			}
			
			$( '.module-carousel' ).each( function() {

				if ( $( this ).find( '.entry' ).length < parseInt( BronzeParams.relatedProductCount, 10 ) + 1 ) {
					return;
				}
