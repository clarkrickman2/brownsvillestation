	...
	return {
		...
		loadMorePosts : function () {
			...
			$( document ).on( 'click', '.loadmore-button', function( event ) {
				...
				if ( BronzeParams.isCustomizer ) {
					event.stopPropagation();
					alert( BronzeParams.l10n.infiniteScrollDisabledMsg );
					return;
				}
				...
				$button.html( BronzeParams.l10n.infiniteScrollMsg ); // loading message
		...
		processContent : function ( response, $button ) {
			...
			if ( response ) {
				...
				if ( BronzeParams.doLoadMorePaginationHashChange ) {
				...
				if ( max < nextPage || undefined === nextPage || isNaN( nextPage ) ) {
					...
					$button.html( BronzeParams.l10n.infiniteScrollEndMsg );
					...
					$.post( BronzeParams.ajaxUrl, {

						action: 'bronze_ajax_get_next_page_link',
						href : $button.attr( 'href' )
						...
						$button.html( BronzeParams.l10n.loadMoreMsg );
					} );
				}
		...
		scrollToPoint : function ( scrollPoint ) {
			...
			$( 'html, body' ).stop().animate( {
				...
				scrollTop: scrollPoint - BronzeUi.getToolBarOffset()
		...
		callBack : function ( $container ) {
			...
			if ( 'undefined' !== typeof BronzeUi ) {
				BronzeUi.adjustmentClasses();
				BronzeUi.resizeVideoBackground();
				BronzeUi.lazyLoad();
				BronzeUi.fluidVideos( $container, true );
				BronzeUi.flexSlider();
				BronzeUi.lightbox();
				BronzeUi.addItemAnimationDelay();
				BronzeUi.parallax();
				BronzeUi.setInternalLinkClass();
				BronzeUi.muteVimeoBackgrounds();
			...
			/* YT background */
			if ( 'undefined' !== typeof BronzeYTVideoBg ) {
				BronzeYTVideoBg.init( $container );
				BronzeYTVideoBg.playVideo( $container );
			}

			if ( $( '.masonry-container' ).length || $( '.metro-container' ).length ) {
				BronzeMasonry.masonry();
				BronzeMasonry.resizeTimer();
