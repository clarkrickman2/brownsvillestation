...
if ( $work_type_include ) {
	...
	$work_types_array = bronze_list_to_array( $work_type_include );
...
if ( $work_type_exclude ) {
	...
	$work_types_ids = array();
	$work_types_array = bronze_list_to_array( $work_type_exclude );
...
$portfolio_url = ( function_exists( 'bronze_get_portfolio_url' ) && bronze_get_portfolio_url() ) ? bronze_get_portfolio_url() : home_url();

$all_active_class = ( ! is_tax( 'work_type' ) ) ? 'active' : '';
?>
<div class="<?php echo bronze_sanitize_html_classes( $filter_class ); ?>">
	<ul>
		<li><a class="filter-link <?php echo esc_attr( $all_active_class ); ?>" href="<?php echo esc_url( $portfolio_url ); ?>" data-filter="work"><?php esc_html_e( 'All', 'bronze' ) ?></a></li>
