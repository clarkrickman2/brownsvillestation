...
if ( $band_include ) {
	...
	$bands_ids = array();
	$bands_array = bronze_list_to_array( $band_include );
...
if ( $band_exclude ) {
	...
	$bands_ids = array();
	$bands_array = bronze_list_to_array( $band_exclude );
...
<div class="<?php echo bronze_sanitize_html_classes( $filter_class ); ?>">
	<ul>
		<li><a class="filter-link <?php echo esc_attr( $all_active_class ); ?>" href="<?php echo esc_url( $releases_url  ); ?>" data-filter="release"><?php esc_html_e( 'All', 'bronze' ) ?></a></li>
