...
if ( $video_type_include ) {
	...
	$video_types_array = bronze_list_to_array( $video_type_include );
...
if ( $video_type_exclude ) {
	...
	$video_types_ids = array();
	$video_types_array = bronze_list_to_array( $video_type_exclude );
...
$videos_url = ( bronze_get_videos_url() ) ? bronze_get_videos_url() : home_url();

$all_active_class = ( ! is_tax( 'video_type' ) ) ? 'active' : '';
?>
<div class="<?php echo bronze_sanitize_html_classes( $filter_class ); ?>">
	<ul>
		<li><a class="filter-link <?php echo esc_attr( $all_active_class ); ?>" href="<?php echo esc_url( $videos_url  ); ?>" data-filter="video"><?php esc_html_e( 'All', 'bronze' ) ?></a></li>
