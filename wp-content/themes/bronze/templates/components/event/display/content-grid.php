		...
		<div class="entry-container">
			<a class="entry-link" href="<?php the_permalink(); ?>">
				...
				<div class="entry-inner">
					<?php if ( $soldout ) : ?>
						<span class="sold-out-label"><?php esc_html_e( 'Sold Out', 'bronze' ); ?></span>
					<?php endif; ?>
					<?php if ( $cancelled ) : ?>
						<span class="cancelled-label"><?php esc_html_e( 'Cancelled', 'bronze' ); ?></span>
					<?php endif; ?>
					<?php
					...
					<div class="entry-summary">
						<div class="event-date">
							<?php if ( $formatted_start_date ) : ?>
								<span class="start-date <?php echo esc_attr( apply_filters( 'bronze_event_grid_entry_date_class', 'wvc-bigtext' ) ); ?>" itemprop="startDate" content="<?php echo esc_attr( $raw_start_date ); ?>">
									<?php echo we_sanitize_date( $formatted_start_date ); ?>
								</span>
							<?php endif; ?>
							<?php if ( $formatted_end_date ) : ?>
								<span class="end-date <?php echo esc_attr( apply_filters( 'bronze_event_grid_entry_date_class', 'wvc-bigtext' ) ); ?>" itemprop="endDate" content="<?php echo esc_attr( $raw_end_date ); ?>">
									<?php echo we_sanitize_date( $formatted_end_date ); ?>
								</span>
							<?php endif; ?>
						</div><!-- .event-date -->
						<div class="event-location" itemprop="location" itemscope itemtype="https://schema.org/<?php echo esc_attr( apply_filters( 'bronze_microdata_event_itemtype_venue', 'MusicVenue' ) ); ?>">
							...
							<h3 class="location-title <?php echo esc_attr( apply_filters( 'bronze_event_grid_entry_title_class', '' ) ); ?>"><span><?php

							if ( 'location' === $event_location ) {
								
								echo sanitize_text_field( $display_location );
								...
								echo sanitize_text_field( $venue );
							}
